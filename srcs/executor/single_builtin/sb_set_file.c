...
/*   Updated: 2022/06/06 20:56:27 by kahirose         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "executor.h"

bool	is_valid_file_sb(char *file_name)
{
...
{
	proc_info->hrdc_info = (t_heredoc_info *)ft_x_calloc(1, sizeof(t_heredoc_info));
	proc_info->hrdc_info->buffer = ast_node->buffer;
	proc_info->is_here_doc = true;
}

bool	put_file_err_msg(char *file_name, t_process_info *proc_info)
{
	safe_func(write(1, "minishell: ", 11));
	perror(file_name);
	return (false);
	proc_info = NULL;//unused用
}
...
bool	sb_set_out_file(t_process_info *proc_info, char *file_name, int redirect_type)
{
	...
	if (redirect_type == OUT)
	{
		...
		if (open_fd == -1)
			return (put_file_err_msg(file_name, proc_info));
